...
windows_zabbix_agent_conf:
  file.managed:
    ...
    - contents: |
        ...
        # Default:
        # Server=
    	{%- if passive_host and passive_host.get('host') %}
        Server={{ passive_host.get('host') }}
    	{%- endif %}
        ### Option: ListenPort
        ...
        # Range: 1024-32767
        # Default:
	{%- if passive_host and passive_host.get('port') %}
        ListenPort={{ passive_host.get('port') }}
	{%- endif %}
        ...
        ServerActive={{ active_host.get('host') }}:{{ active_host.get('port') }}
